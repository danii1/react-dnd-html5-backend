...
var HTML5Backend = (function () {
  ...
  HTML5Backend.prototype.handleTopDragStart = function handleTopDragStart(e) {
    ...
    try {
      nativeType = _NativeDragSources.matchNativeItemType(dataTransfer);
    } catch (err) {
  ...
  HTML5Backend.prototype.handleTopDragLeaveCapture = function handleTopDragLeaveCapture(e) {
    if (this.isDraggingNativeItem()) {
      ...
      try {
        ...
        'Error when trying to access e.preventDefault in handleTopDragLeaveCapture', this.monitor.getItemType());
      }
    }

    var isLastLeave = false;
    // #FirefoxPermissionDenied
    try {
      isLastLeave = this.enterLeaveCounter.leave(e.target);
    } catch (err) {
      host && host.recordWarning && host.recordWarning( // eslint-disable-line
      'Error when trying to access e.target in handleTopDragLeaveCapture', this.monitor.getItemType());
    }
    if (!isLastLeave) {
      return;
    }
